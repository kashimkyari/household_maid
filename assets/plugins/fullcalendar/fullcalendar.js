...
// (such as highlight rows, day rows, maid rows, etc).
...
var RowRenderer = Class.extend({
	...
	// `rowType` (like day, eventSkeleton, maidSkeleton), which is optional.
...
var Grid = fc.Grid = RowRenderer.extend({
	...
	renderRangemaid: function(range, sourceSeg) {
		var fakeEvent;
		...
		fakeEvent.className = (fakeEvent.className || []).concat('fc-maid');
		...
		this.rendermaid(fakeEvent, sourceSeg); // do the actual rendering
	},


	// Renders a mock event
	rendermaid: function(event, sourceSeg) {
		// subclasses must implement
	},


	// Unrenders a mock event
	destroymaid: function() {
		// subclasses must implement
	},
...
Grid.mixin({
			...
			cellOver: function(cell, isOrig) {
				...
				if (dropLocation) {
					if (view.renderDrag(dropLocation, seg)) { // have the subclass render a visual indication
						mouseFollower.hide(); // if the subclass is already using a mock event "maid", hide our own
					}
					else {
						mouseFollower.show();
					}
					...
					// have the maid follow the mouse (no snapping) with a warning-style cursor
					mouseFollower.show();
					disableCursor();
				}
			},
	...
	// A truthy returned value indicates this method has rendered a maid element.
...
var DayGrid = Grid.extend({
	...
	maidEls: null, // set of cell skeleton elements for rendering the mock event "maid"
	...
	renderDrag: function(dropLocation, seg) {
		...
		// if a segment from the same calendar but another component is being dragged, render a maid event
		if (seg && !seg.el.closest(this.el).length) {

			this.renderRangemaid(dropLocation, seg);

			opacity = this.view.opt('dragOpacity');
			if (opacity !== undefined) {
				this.maidEls.css('opacity', opacity);
			}

			return true; // a maid has been rendered
		}
	},
	...
	renderEventResize: function(range, seg) {
		this.renderHighlight(range);
		this.renderRangemaid(range, seg);
	},
	...
	// Renders a mock "maid" event. `sourceSeg` is the associated internal segment object. It can be null.
	rendermaid: function(event, sourceSeg) {
		var maidNodes = [];
		...
		this.rowEls.each(function(row, rowNode) {
			...
			var skeletonEl = $('<div class="fc-maid-skeleton"><table/></div>'); // will be absolutely positioned
			var skeletonTop;
			...
			maidNodes.push(skeletonEl[0]);
		});

		this.maidEls = $(maidNodes); // array -> jQuery set
	},


	// Unrenders any visual indication of a mock maid event
	destroymaid: function() {
		if (this.maidEls) {
			this.maidEls.remove();
			this.maidEls = null;
...
var TimeGrid = Grid.extend({
	...
	maidEl: null, // cell skeleton element for rendering the mock event "maid"
	...
	// A returned value of `true` signals that a mock "maid" event has been rendered.
	renderDrag: function(dropLocation, seg) {
		var opacity;

		if (seg) { // if there is event information for this drag, render a maid event
			this.renderRangemaid(dropLocation, seg);

			opacity = this.view.opt('dragOpacity');
			if (opacity !== undefined) {
				this.maidEl.css('opacity', opacity);
			}

			return true; // signal that a maid has been rendered
		}
		else {
	...
	// Renders a mock "maid" event. `sourceSeg` is the original segment object and might be null (an external drag)
	rendermaid: function(event, sourceSeg) {
		var segs = this.eventsToSegs([ event ]);
		var tableEl;
		var i, seg;
		...
		this.maidEl = $('<div class="fc-maid-skeleton"/>')
			.append(tableEl)
				.appendTo(this.el);
	},


	// Unrenders any mock maid event
	destroymaid: function() {
		if (this.maidEl) {
			this.maidEl.remove();
			this.maidEl = null;
	...
	renderSelection: function(range) {
		if (this.view.opt('selectmaid')) { // this setting signals that a mock maid event should be rendered
			this.renderRangemaid(range);
		}
		else {
			this.renderHighlight(range);
		}
...
var BasicView = fcViews.basic = View.extend({
	...
	// Affects maid-skeleton and highlight-skeleton rows.
	introHtml: function() {
		if (this.weekNumbersVisible) {
...
fcViews.agenda = View.extend({ // AgendaView
	...
	// Affects content-skeleton, maid-skeleton, highlight-skeleton for both the time-grid and day-grid.
